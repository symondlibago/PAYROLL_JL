...
const InputGroup = ({ label, value, onChange, placeholder = "0", type = "number", className = "", readOnly = false }) => (
  <div className={`space-y-1 ${className}`}>
    ...
    <input
      ...
      readOnly={readOnly}
      className={`w-full px-2 py-1.5 border border-gray-200 rounded text-sm transition-all text-right
        ${readOnly ? 'bg-gray-100 text-gray-600 cursor-not-allowed' : 'bg-white focus:ring-1 focus:ring-blue-500 focus:border-blue-500'}
      `}
...
export const ProcessPayrollModal = ({ isOpen, onClose, onSubmit, employees, isLoading, initialData }) => {
  const [formData, setFormData] = useState({
    ...
    // NEW EARNINGS FIELDS
    ecola: '', adjustment_1: '', adjustment_2: '',

    sss_deduction: '', philhealth_deduction: '', pagibig_deduction: '',
    // EMPLOYER SHARES (Not submitted directly, but calculated for display)
    sss_employer_share: '', philhealth_employer_share: '', pagibig_employer_share: '',
  ...
  const preview = useMemo(() => {
    ...
    const ecola = getVal('ecola')
    const adj1 = getVal('adjustment_1')
    const adj2 = getVal('adjustment_2')

    const gross = basic + holiday + nd + ot + allowance + ecola + adj1 + adj2
  ...
  // Helper to safely multiply for display
  const getEmployerShare = (val, multiplier) => {
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white p-5 rounded-xl border border-gray-200">
                  ...
                  <div className="space-y-3">
                    <DualInputGroup label="Sunday / Rest Day (130%)" dayValue={formData.sunday_rest_day_days} onDayChange={v => handleChange('sunday_rest_day_days', v)} hourValue={formData.sunday_rest_day_hours} onHourChange={v => handleChange('sunday_rest_day_hours', v)} />
                    <DualInputGroup label="Special Day (130%)" dayValue={formData.special_day_days} onDayChange={v => handleChange('special_day_days', v)} hourValue={formData.special_day_hours} onHourChange={v => handleChange('special_day_hours', v)} />
                    <DualInputGroup label="Spc. Day on Rest Day (150%)" dayValue={formData.special_day_rest_day_days} onDayChange={v => handleChange('special_day_rest_day_days', v)} hourValue={formData.special_day_rest_day_hours} onHourChange={v => handleChange('special_day_rest_day_hours', v)} />
                    <DualInputGroup label="Regular Holiday (200%)" dayValue={formData.regular_holiday_days} onDayChange={v => handleChange('regular_holiday_days', v)} hourValue={formData.regular_holiday_hours} onHourChange={v => handleChange('regular_holiday_hours', v)} />
                    <DualInputGroup label="Reg. Hol on Rest Day (260%)" dayValue={formData.regular_holiday_rest_day_days} onDayChange={v => handleChange('regular_holiday_rest_day_days', v)} hourValue={formData.regular_holiday_rest_day_hours} onHourChange={v => handleChange('regular_holiday_rest_day_hours', v)} />
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="bg-white p-5 rounded-xl border border-gray-200">
                    ...
                    <div className="space-y-3">
                      <DualInputGroup label="Ordinary (110%)" dayValue={formData.nd_ordinary_days} onDayChange={v => handleChange('nd_ordinary_days', v)} hourValue={formData.nd_ordinary_hours} onHourChange={v => handleChange('nd_ordinary_hours', v)} />
                      <DualInputGroup label="Rest/Special (143%)" dayValue={formData.nd_rest_special_days} onDayChange={v => handleChange('nd_rest_special_days', v)} hourValue={formData.nd_rest_special_hours} onHourChange={v => handleChange('nd_rest_special_hours', v)} />
                      <DualInputGroup label="Reg Holiday (220%)" dayValue={formData.nd_regular_holiday_days} onDayChange={v => handleChange('nd_regular_holiday_days', v)} hourValue={formData.nd_regular_holiday_hours} onHourChange={v => handleChange('nd_regular_holiday_hours', v)} />
                    </div>
                  </div>
            ...
            <div className="xl:col-span-4 space-y-6">
              ...
              {/* ALLOWANCE & NEW FIELDS */}
              <div className="bg-green-50/50 p-5 rounded-xl border border-green-100">
                <SectionHeader icon={Banknote} title="Additional Earnings" color="green" />
                <div className="space-y-3">
                  <InputGroup label="Allowance" value={formData.allowance_amount} onChange={v => handleChange('allowance_amount', v)} />
                  <InputGroup label="Allowance Remarks" type="text" value={formData.allowance_remarks} onChange={v => handleChange('allowance_remarks', v)} placeholder="e.g. Rice Subsidy" />
                  
                  <div className="border-t border-green-200 my-2"></div>
                  
                  {/* NEW FIELDS */}
                  <InputGroup label="ECOLA" value={formData.ecola} onChange={v => handleChange('ecola', v)} />
                  <InputGroup label="Adjustment 1" value={formData.adjustment_1} onChange={v => handleChange('adjustment_1', v)} />
                  <InputGroup label="Adjustment 2" value={formData.adjustment_2} onChange={v => handleChange('adjustment_2', v)} />
                </div>
              </div>
              ...
              <div className="bg-red-50/50 p-5 rounded-xl border border-red-100">
                ...
                <div className="space-y-4">
                  ...
                  {/* GOVERNMENT DEDUCTIONS (SIDE BY SIDE) */}
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-red-700 uppercase block">Government Shares</label>
                    
                    {/* SSS */}
                    <div className="grid grid-cols-2 gap-2">
                      <InputGroup label="SSS (EE)" value={formData.sss_deduction} onChange={v => handleChange('sss_deduction', v)} />
                      <InputGroup label="SSS (ER)" value={getEmployerShare(formData.sss_deduction, 2)} readOnly={true} onChange={() => {}} />
                    </div>

                    {/* PhilHealth */}
                    <div className="grid grid-cols-2 gap-2">
                      <InputGroup label="PhilHealth (EE)" value={formData.philhealth_deduction} onChange={v => handleChange('philhealth_deduction', v)} />
                      <InputGroup label="PhilHealth (ER)" value={getEmployerShare(formData.philhealth_deduction, 1)} readOnly={true} onChange={() => {}} />
                    </div>

                    {/* Pag-IBIG */}
                    <div className="grid grid-cols-2 gap-2">
                      <InputGroup label="Pag-IBIG (EE)" value={formData.pagibig_deduction} onChange={v => handleChange('pagibig_deduction', v)} />
                      <InputGroup label="Pag-IBIG (ER)" value={getEmployerShare(formData.pagibig_deduction, 1)} readOnly={true} onChange={() => {}} />
                    </div>
                  </div>

                  {/* OTHER DEDUCTIONS */}
                  <div className="grid grid-cols-2 gap-3 pt-2 border-t border-red-200/50">
                  ...
                  <div className="pt-2 border-t border-red-200/50">
                    <label className="text-[10px] font-bold text-red-700 uppercase mb-2 block">Loans</label>
                    <div className="grid grid-cols-2 gap-3">
...
// ... ViewPayrollModal remains mostly unchanged, you can add new fields there if you want them displayed in view mode ... 
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          <div>
            ...
            <div className="space-y-1">
              ...
              {(parseFloat(payroll.holiday_pay) > 0) && <DetailRow label="Holiday & Premiums" value={parseFloat(payroll.holiday_pay)} />}
              ...
              {(parseFloat(payroll.ecola) > 0) && <DetailRow label="ECOLA" value={parseFloat(payroll.ecola)} />}
              {(parseFloat(payroll.adjustment_1) > 0) && <DetailRow label="Adjustment 1" value={parseFloat(payroll.adjustment_1)} />}
              {(parseFloat(payroll.adjustment_2) > 0) && <DetailRow label="Adjustment 2" value={parseFloat(payroll.adjustment_2)} />}
